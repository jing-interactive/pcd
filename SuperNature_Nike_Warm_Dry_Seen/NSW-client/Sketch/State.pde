...
public class State {
    ...
    void tryFadeOutEverything(float time, float duration, float fadeoutSeconds) {
        if (time > duration - fadeoutSeconds && alphaEverything > 254) {
            println("time: " + time);
            alphaEverything = 254;
            fadeOutAlpha("alphaEverything", fadeoutSeconds);
        }
